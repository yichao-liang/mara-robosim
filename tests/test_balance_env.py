"""Tests for the PyBulletBalanceEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletBalanceEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletBalanceEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_balance"
...
def test_types(env):
    ...
    assert "machine" in type_names
    assert "plate" in type_names
...
def test_predicates(env):
    ...
    assert "Balanced" in pred_names
    assert "Holding" in pred_names
    assert "MachineOn" in pred_names

"""Tests for the PyBulletAntsEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletAntsEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletAntsEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_ants"


def test_types(env):
    """types is a non-empty set containing expected type names."""
    type_names = {t.name for t in env.types}
    assert "ants" in type_names
...
def test_predicates(env):
    ...
    assert "Attractive" in pred_names
    assert "Holding" in pred_names
    assert "InSortedRegion" in pred_names

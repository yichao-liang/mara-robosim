"""Tests for the PyBulletLaserEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletLaserEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletLaserEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_laser"

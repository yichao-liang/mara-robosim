"""Tests for the PyBulletFanEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletFanEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletFanEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_fan"


def test_types(env):
    """types is a non-empty set containing expected type names."""
    type_names = {t.name for t in env.types}
    assert "ball" in type_names
    assert "fan" in type_names
...
def test_predicates(env):
    ...
    assert "FanOn" in pred_names
    assert "FanOff" in pred_names
    assert "BallAtLoc" in pred_names

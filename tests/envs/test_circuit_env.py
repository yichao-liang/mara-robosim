"""Tests for the PyBulletCircuitEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletCircuitEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletCircuitEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_circuit"

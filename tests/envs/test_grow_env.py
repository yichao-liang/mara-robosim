"""Tests for the PyBulletGrowEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletGrowEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletGrowEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_grow"

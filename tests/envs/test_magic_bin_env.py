"""Tests for the PyBulletMagicBinEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletMagicBinEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletMagicBinEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_magic_bin"


def test_types(env):
    """types is a non-empty set containing expected type names."""
    type_names = {t.name for t in env.types}
    assert "bin" in type_names
...
def test_predicates(env):
    ...
    assert "InBin" in pred_names
    assert "Vanished" in pred_names

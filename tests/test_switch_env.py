"""Tests for the PyBulletSwitchEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletSwitchEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletSwitchEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_switch"

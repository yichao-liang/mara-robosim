"""Tests for the PyBulletBlocksEnv environment."""
...
def env():
    ...
    try:
        ...
        return PyBulletBlocksEnv(config=config, use_gui=False)
    except Exception as exc:
        pytest.skip(f"Cannot instantiate PyBulletBlocksEnv: {exc}")


def test_name(env):
    """get_name() returns the expected identifier."""
    assert env.get_name() == "pybullet_blocks"

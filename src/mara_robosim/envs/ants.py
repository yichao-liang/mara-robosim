"""PyBullet Ants environment.

A robot sorts food blocks by whether they attract ants. Ants move toward
attractive food blocks with noise.
"""
...
class PyBulletAntsEnv(PyBulletEnv):
    ...
    food_half_extents: ClassVar[Tuple[float, float, float]] = (0.03, 0.03, 0.03)
    food_size: ClassVar[float] = food_half_extents[0] * 2
    food_mass: ClassVar[float] = 0.1

    # Ant shape
    ant_half_extents: ClassVar[Tuple[float, float, float]] = (0.015, 0.01, 0.01)
    ...
    _food_type = Type(
        "food",
        ["x", "y", "z", "rot", "is_held", "attractive", "r", "g", "b"],
    ...
    def __init__(
        ...
        self._Holding = Predicate(
            "Holding", [self._robot_type, self._food_type], self._Holding_holds
        )
        self._HandEmpty = Predicate(
            "HandEmpty", [self._robot_type], self._HandEmpty_holds
        )
        self._On = Predicate("On", [self._food_type, self._food_type], self._On_holds)
        self._OnTable = Predicate("OnTable", [self._food_type], self._OnTable_holds)
        self._Clear = Predicate("Clear", [self._food_type], self._Clear_holds)
        self._InSortedRegion = Predicate(
            "InSortedRegion", [self._food_type], self._InSortedRegion_holds
        )
        self._Attractive = Predicate(
            "Attractive", [self._food_type], self._Attractive_holds
        )
    ...
    def _reset_custom_env_state(self, state: State) -> None:
        ...
        for food in state.get_objects(self._food_type):
            ...
            update_object(
                food.id, color=(r, g, b, 1.0), physics_client_id=self._physics_client_id
            )
    ...
    def _On_holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        return np.allclose([x1, y1, z1], [x2, y2, z2 + self.food_size], atol=0.01)
    ...
    def _InSortedRegion_holds(self, state: State, objects: Sequence[Object]) -> bool:
    ...
    def _make_tasks(
        ...
        for _ in range(num_tasks):
            ...
            raw_palette = rng.choice(
                self._obj_colors_main, size=self.num_colors, replace=False
            )
            task_color_palette: List[Tuple[float, ...]] = [
                tuple(c) for c in raw_palette
            ]
            raw_attractive = rng.choice(
                task_color_palette, size=self.num_attractive_colors, replace=False
            )
            ...
            for i in range(num_blocks):
                ...
                init_dict[fobj] = {
                    "x": pos[0],
                    "y": pos[1],
                    "z": self.z_lb + self.food_half_extents[2],
                    "rot": rot,
                    "is_held": 0.0,
            ...
            for c, blocks_group in block_by_color.items():
                goal_atoms.add(GroundAtom(self._InSortedRegion, [blocks_group[0]]))
                if len(blocks_group) > 1:
                    for j in range(len(blocks_group) - 1):
                        goal_atoms.add(
                            GroundAtom(self._On, [blocks_group[j + 1], blocks_group[j]])
                        )

...
class PyBulletFloatEnv(PyBulletEnv):
    ...
    _block_type = Type(
        "block", ["x", "y", "z", "in_water", "is_held"], sim_features=["id", "is_light"]
    )
    ...
    def __init__(
        ...
        self._InWater = Predicate("InWater", [self._block_type], self._InWater_holds)
    ...
    def _store_pybullet_bodies(self, pybullet_bodies: Dict[str, Any]) -> None:
        ...
        for i, (blk, id) in enumerate(zip(self._blocks, pybullet_bodies["block_ids"])):
            if i == num_blocks - 1:
                blk.is_light = 1.0
            else:
                blk.is_light = 0.0
    ...
    def _float_light_blocks(self, state: State) -> None:
        ...
        for blk in self._blocks:
            ...
            if self._is_in_left_compartment(bx, by) or self._is_in_right_compartment(
                bx, by
            ):
    ...
    def _update_water_level_if_needed(self, state: State) -> bool:
        ...
        for blk in self._blocks:
            ...
            # Condition for "entering water" => block top is below water
            # surface (i.e. the entire block is now submerged)
            if top_z < surface_z:
        ...
        # we had, the water height won't change. But we don't store old
        # displaced volume separately; we store old_height. We can compute old
        # displaced from (old_volume - 2*area*lowest_level_of_water).
        # Easiest approach: just see how many blocks were displacing before
        # vs. now.
        ...
        if old_num_displacing == new_num_displacing:
            ...
            # the net displacement is the same. So no water level change
            # either.
            changed = False
        else:
            ...
            new_displaced_vol = (
                new_num_displacing * (self.block_size**3) * increase_factor
            )
    ...
    def _make_tasks(
        ...
        for _ in range(num_tasks):
            ...
            block_xy_positions = sample_collision_free_2d_positions(
                ...
                x_range=(
                    self.VESSEL_BASE_X + self.CONTAINER_OPENING_LEN + self.block_size,
                    self.VESSEL_BASE_X
                    + self.CONTAINER_OPENING_LEN
                    + self.CONTAINER_GAP
                    - self.block_size,
            ...
            # Adding z values
            block_positions = [
                (pos[0], pos[1], self.z_lb + self.block_size / 2)
            ...
            goal_atoms = {GroundAtom(self._InWater, [b]) for b in first_two_blocks}
            goal_atoms.add(GroundAtom(self._Holding, [self._robot, self._blocks[2]]))

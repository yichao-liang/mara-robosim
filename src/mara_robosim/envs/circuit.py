...
class PyBulletCircuitEnv(PyBulletEnv):
    ...
    _bulb_on_color: ClassVar[Tuple[float, float, float, float]] = (
        1.0,
        1.0,
        0.0,
        1.0,
    )  # yellow
    _bulb_off_color: ClassVar[Tuple[float, float, float, float]] = (
        0.8,
        0.8,
        0.8,
        1.0,
    ...
    def __init__(
        ...
        self._Holding = Predicate(
            "Holding", [self._robot_type, self._wire_type], self._Holding_holds
        )
        self._HandEmpty = Predicate(
            "HandEmpty", [self._robot_type], self._HandEmpty_holds
        )
        self._ConnectedToLight = Predicate(
            "ConnectedToLight",
        ...
        self._ConnectedToBattery = Predicate(
            "ConnectedToBattery",
            [self._wire_type, self._switch_box_type],
        ...
        self._SwitchedOn = Predicate(
            "SwitchedOn", [self._switch_box_type], self._SwitchedOn_holds
        )
    ...
    def initialize_pybullet(
        ...
        if getattr(config, "circuit_battery_in_box", False):
            ...
            battery_urdf = (
                "urdf/partnet_mobility/switch/102812/battery_box_switch_snap.urdf"
            )
    ...
    def step(self, action: Action, render_obs: bool = False) -> State:
        ...
        if (
            ...
            both_batteries_in_box = self._InBatteryBox_holds(
                next_state, [self._c_battery1]
    ...
    def _ConnectedToLight_holds(state: State, objects: Sequence[Object]) -> bool:
        ...
        target_x_diff = (
            PyBulletCircuitEnv.wire_snap_length / 2 - PyBulletCircuitEnv.snap_width / 2
        )
        target_y_diff = (
            PyBulletCircuitEnv.bulb_snap_length / 2 + PyBulletCircuitEnv.snap_width / 2
        )
        ...
        if (
            x_diff < min_x_diff
            or x_diff > max_x_diff
            or y_diff < min_y_diff
    ...
    def _CircuitClosed_holds(state: State, objects: Sequence[Object]) -> bool:
        ...
        for wire in wires:
            if not PyBulletCircuitEnv._ConnectedToLight_holds(state, [wire, light]):
                return False
            if not PyBulletCircuitEnv._ConnectedToBattery_holds(state, [wire, battery]):
                return False
        return True
    ...
    def _InBatteryBox_holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        box_x_min = (
            switch_box_x - self.battery_box_x_offset - self.battery_box_x_width / 2
        )
        box_x_max = (
            switch_box_x - self.battery_box_x_offset + self.battery_box_x_width / 2
        )
    ...
    @staticmethod
    def _CircuitClosed_CP_holds(
        atoms: Set[GroundAtom], objects: Sequence[Object]
    ) -> bool:
        wire1, wire2 = objects
        if wire1 == wire2:
            return False
        ...
        return (
            w1_connected_to_light
            and w1_connected_to_battery
            and w2_connected_to_light
            and w2_connected_to_battery
        )
    ...
    def _is_bulb_on(self, light_id: int) -> bool:
        ...
        color = p.getVisualShapeData(light_id, physicsClientId=self._physics_client_id)[
            3
        ][-1]
    ...
    def _make_tasks(
        ...
        for _ in range(num_tasks):
            ...
            battery_x = rng.uniform(
                self.x_lb + 2 * self.init_padding, self.x_lb + 6 * self.init_padding
            )
            ...
            # Wires
            wire1_x = rng.uniform(
                battery_x + self.wire_snap_length / 2, battery_x + self.wire_snap_length
            )
            wire1_dict = {
                "x": wire1_x,
                "y": 1.15,  # lower region
            ...
            goal_atoms = {
                ...
                GroundAtom(self._ConnectedToBattery, [self._wire2, self._battery]),
            }
...
if __name__ == "__main__":
    ...
    config = PyBulletConfig(seed=0, num_train_tasks=1)
    env = PyBulletCircuitEnv(config=config, use_gui=True)

...
from mara_robosim.pybullet_helpers.link import get_link_pose
...
def pybullet_inverse_kinematics(
    ...
    ----------
    ik_tol : float
        Convergence tolerance for the end-effector position.
    max_ik_iters : int
        Maximum number of IK iterations before raising an error.
    """
    ...
    for _ in range(max_ik_iters):
        ...
        if np.allclose(ee_link_pose.position, target_position, atol=convergence_tol):
            break
    else:

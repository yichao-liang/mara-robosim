...
from mara_robosim.pybullet_helpers.geometry import (
    Pose,
    matrix_from_quat,
    multiply_poses,
)
from mara_robosim.pybullet_helpers.ikfast.load import import_ikfast
from mara_robosim.pybullet_helpers.joint import (
    JointInfo,
    JointPositions,
...
from mara_robosim.pybullet_helpers.link import get_link_pose, get_relative_link_pose
...
def ikfast_closest_inverse_kinematics(
    robot: SingleArmPyBulletRobot,
    world_from_target: Pose,
    ikfast_max_time: float = 0.05,
    ikfast_max_distance: float = 0.5,
    ikfast_max_attempts: int = 100,
    ikfast_max_candidates: float = 100.0,
    ikfast_norm: int = 2,
    seed: int = 0,
) -> List[JointPositions]:
    ...
    ikfast_max_time: Maximum time in seconds for IKFast search.
    ikfast_max_distance: Maximum joint-space distance from current position.
    ikfast_max_attempts: Maximum number of free-joint sampling attempts.
    ikfast_max_candidates: Maximum number of IK candidate solutions.
    ikfast_norm: Norm order for distance computation.
    seed: Random seed.
    ...
    if not (
        ...
        or ikfast_max_attempts < np.inf
        or ikfast_max_candidates < np.inf
    ):
        raise ValueError(
            "At least one of ikfast_max_time, ikfast_max_attempts, "
            "ikfast_max_candidates must be finite."
        )
    ...
    generator = ikfast_inverse_kinematics(
        robot,
        world_from_target,
        max_time=ikfast_max_time,
        max_distance=ikfast_max_distance,
        max_attempts=ikfast_max_attempts,
        norm=ikfast_norm,
        rng=np.random.default_rng(seed),
    )

    # Only use up to the max candidates specified
    if ikfast_max_candidates < np.inf:  # pragma: no cover
        generator = islice(generator, int(ikfast_max_candidates))

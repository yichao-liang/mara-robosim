...
from mara_robosim.pybullet_helpers.ikfast.utils import ikfast_closest_inverse_kinematics
...
from mara_robosim.pybullet_helpers.link import BASE_LINK, get_link_state
from mara_robosim.structs import Array
...
class SingleArmPyBulletRobot(abc.ABC):
    ...
    def reset_state(self, robot_state: Array) -> None:
        ...
        for finger_id in [self.left_finger_id, self.right_finger_id]:
            p.resetJointState(
                self.robot_id, finger_id, rf, physicsClientId=self.physics_client_id
            )
    ...
    def set_motors(
        self, joint_positions: JointPositions, control_mode: str = "position"
    ) -> None:
        ...
        ----------
        control_mode : str
            Either "position" (uses POSITION_CONTROL) or "reset"
            (directly resets joint states).
        """
        assert len(joint_positions) == len(self.arm_joints)
        ...
        if control_mode == "position":
            ...
            raise NotImplementedError(
                "Unrecognized pybullet_control_mode: " f"{control_mode}"
            )
    ...
    def _validate_joints_state(
        self, joint_positions: JointPositions, target_pose: Pose, ik_tol: float = 1e-3
    ) -> None:
        ...
        pos_is_close = np.allclose(ee_pos, target_pos, atol=ik_tol)
    ...
    def inverse_kinematics(
        self, end_effector_pose: Pose, validate: bool, set_joints: bool = True
    ) -> JointPositions:
